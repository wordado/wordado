...
- **The web transport** (plan 6): `POST /v1/sync/push` and `POST /v1/sync/pull` with `credentials: 'include'` and `content-type: application/json`; any non-200 is a thrown, retryable failure, exactly as `client.test.ts`'s `transportFor` does. `upgrade_required` arrives as a 200 body. A 400 means the client built a malformed page — a bug to fix, not to retry forever; plan 6 should surface it in the sync status. In development, plan 6's Vite dev server proxies `/api` and `/v1` to the Worker on :8787, so the app and the API share one origin and the server needs no CORS; `TRUSTED_ORIGINS` covers the `Origin` header the proxy forwards.
- **Sign-in** (plan 6): Better Auth's client (`better-auth/client` with `emailOTPClient`) against `/api/auth`: `emailOtp.sendVerificationOtp({ email, type: 'sign-in' })`, then `signIn.emailOtp({ email, otp })`; Google through `signIn.social({ provider: 'google' })`. After the age gate, set the country with `updateUser({ country })` — a two-letter code or null, anything else is a 400 — pre-filled from `GET /v1/country`. A 429 `{ "error": "sign_in_email_limit" }` from the code request means no code was sent (three an hour per address, a hundred a day in all); plan 6 should say so rather than wait for an email. `GET /v1/me` names the signed-in learner.
...
- **Deploy** (plan 7): deploy the server before any client that raises `SYNC_PROTOCOL_VERSION`; a page from a newer protocol is accepted and its unknown fields are dropped. Run `pnpm --filter @wordado/server migrate` with the production `DATABASE_URL` before the Worker; create the Hyperdrive and put its id in `wrangler.jsonc`; create the `wordado-jobs` queue; set `BASE_URL` and `TRUSTED_ORIGINS` per environment; secrets `BETTER_AUTH_SECRET`, `RESEND_API_KEY`, `EMAIL_FROM`, `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY`, `VAPID_SUBJECT`. CI runs the suite against a Postgres 18 service with `TEST_DATABASE_URL` set, and should run `pnpm smoke` too. A rate-limiting rule in front of `/v1/sync/*` must allow a full backlog flush (spec §10).
- **CPU on the preview deployment** (plan 7, spec §4.4): push of a 500-event page, pull of a learner with thousands of words, a full re-derivation in the queue, the export, and per-request `betterAuth()` construction are the paths to measure against the Workers CPU limit. Measure too the Hyperdrive queries per push page (about 20 today; spec §17.2 asks for a few) and the learner-lock tuple write every push and pull makes.
